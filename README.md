...
这个项目把常用 TOTP 验证码整理成可管理的卡片列表。当前页面结构以卡片面板为中心：顶部提供 **显示密钥 / 隐藏密钥**、**导入 JSON**、**导出 JSON** 和 **添加卡片** 按钮，主体区域展示验证码卡片列表；新增与编辑操作通过覆盖层工作区完成，而不是在首页常驻表单中完成。
...
- **密钥显隐控制**：卡片默认隐藏原始密钥，可一键统一显示或再次隐藏
...
- 卡片会展示备注、已隐藏或已显示的密钥文本，以及当前验证码
- 默认会隐藏所有卡片的原始密钥；点击顶部 **显示密钥** 后可统一查看，再次点击 **隐藏密钥** 可恢复遮罩显示
- 点击 **复制** 可把当前验证码写入剪贴板
